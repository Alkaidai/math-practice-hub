  ...
  return (
    <div className="bg-card rounded-xl shadow-sm p-5 border border-border">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-caption text-muted-foreground">{completedCount}/{missions.length} concluídas</span>
      </div>
      <div className="space-y-3">
        {missions.map(m => {
          const progress = Math.min(100, Math.round((m.currentValue / m.targetValue) * 100));
          return (
            <div key={m.id} className={`rounded-xl p-3.5 transition-all ${m.completed ? 'bg-success-soft border border-success/20' : 'bg-muted/50'}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-body">{getMissionEmoji(m.missionType)}</span>
                  <span className={`text-body font-medium ${m.completed ? 'text-success line-through' : 'text-foreground'}`}>
              ...
              <Progress value={progress} className="h-1.5" indicatorClassName={m.completed ? 'bg-success' : 'bg-primary'} />
            </div>
          );
        })}
      </div>
